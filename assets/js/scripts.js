...
jQuery(document).ready(function($) {
  ...
  $('.tablepress thead th').each(function(index, el) {
    var thisVal = $(this).html();
    var html = '<label class="labid' + index + '" for="thid' + index + '">' + thisVal + '</label>'
    $('.products-pricetable--filters-h').append(html);
  });

  $('.products-pricetable--filters select').each(function(index, el) {
    var thisWidth = $(this).width();
    var selector = 'labid' + index;
    $(this).attr('id', selector);
    selector = '.' + selector;
    $(selector).width(thisWidth)
    $('.labid0').width(thisWidth)
  });

  $('.products-pricetable').css('opacity', '1');;
  $('.products-loader').hide('fast');
